...
class ExtrinsicAdjuster:
    def __init__(self, window, img_dir, pointcloud_dir, calib_path):
        ...
        for file_path in glob.glob(os.path.join(pointcloud_dir, f'*{self.pc_ext}')):
        ...
        # del self.pointcloud_file[-1]

        self.file_num = 0
        image_path = os.path.join(img_dir, self.images_file[self.file_num] + self.img_ext)
        pointcloud_path = os.path.join(pointcloud_dir, self.pointcloud_file[self.file_num] + self.pc_ext)
    ...
    def save_img_pointcloud(self):
        ...
        pc_save_dir = os.path.join(os.path.dirname(self.pointcloud_dir), 'selected_pointcloud')
        ...
        pointcloud_path = os.path.join(self.pointcloud_dir, self.pointcloud_file[self.file_num] + self.pc_ext)

        image_save_path = os.path.join(img_save_dir, self.images_file[self.file_num] + self.img_ext)
        pc_save_path = os.path.join(pc_save_dir, self.pointcloud_file[self.file_num] + self.pc_ext)
    ...
    def update_extrinsic(self, _=None):
        ...
        # 过滤掉非地面的点云数据
        # self.pointcloud = proj_pcd2cam.height_filter(self.pointcloud,-1.2,-0.6)
...
def draw_circle(image, u, v, z):
    ...
    for i in range(len(u)):
        ...
        cv2.circle(adjusted_image, (int(u[i]), int(v[i])), radius=10, color=color_255, thickness=-1)
...
def main():
    ...
    img_dir = f'correspond_data/{ros_bag_name}/image'
    pointcloud_dir = f'correspond_data/{ros_bag_name}/pointcloud'
